...
import org.apache.kafka.common.config.{AsyncLogConfigs, ConfigDef, ConfigException, TopicConfig}
...
class KafkaConfig private(doLog: Boolean, val props: util.Map[_, _])
  ...
  // Note: Log Configuration accessors (logSegmentBytes, autoCreateTopicsEnable, etc.) are
  // inherited from AbstractKafkaConfig; they were redeclared here in the bk-kafka branch but
  // that is redundant on trunk and causes "override modifier required" errors. Removed.
